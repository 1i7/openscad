...
if [ "`echo $* | grep 64`" ]; then
 ...
 if [ "`echo $* | grep download`" ]; then
  PACKAGES='download-mpfr download-eigen download-opencsg download-cgal download-qt download-freetype download-fontconfig download-harfbuzz'
 else
  PACKAGES='mpfr eigen opencsg cgal qt freetype fontconfig harfbuzz'
 fi
else
 MXE_TARGETS='i686-pc-mingw32' # fixme - does this work? test it.
 if [ "`echo $* | grep download`" ]; then
  PACKAGES='download-mpfr download-eigen download-opencsg download-cgal download-qt download-freetype download-fontconfig download-harfbuzz download-nsis'
 else
  PACKAGES='mpfr eigen opencsg cgal qt freetype fontconfig harfbuzz nsis'
 fi
fi
